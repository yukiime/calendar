...
public class LS {
    ...
    public static final int[] lunarToSolar(int year, int month, int monthDay, boolean isLeapMonth) {
        ...
        int dayOffset;//用它来储存被计算日期距离从当年阳历1月1日有多少天
        ...
        dayOffset = (LUNAR_INFO[year - MIN_YEAR] & 0x001F) - 1;//取出农历正月初一对应的公历日并-1
        if (((LUNAR_INFO[year - MIN_YEAR] & 0x0060) >> 5) == 2) {//若农历正月初一所对应的公历日期是在2月，则说明1月未被计入天数，因此要将1月计入
            dayOffset += 31;
        }
        for (i = 1; i < month; i++) {//遍历，取出每个月的天数，直到要计算的月的前一个月（不包括用户输入的那个月）
        ...
        dayOffset += monthDay;//加上用户输入的那个月的天数

        leapMonth = ((LUNAR_INFO[year - MIN_YEAR] & 0xf00000) >> 20);//取到该年闰月的月份，若无闰月，则为0

        // 这一年有闰月

        if (leapMonth != 0) {//若当年有闰月，那么前面遍历计数是会少算最后一个整月的天数，所以此处用来加上最后一个整月
            if (month > leapMonth || (month == leapMonth && isLeapMonth)) {//用户输入的阴历月是闰月，或是闰月的后一个月
        ...
        if (dayOffset > 366 || (year % 4 != 0 && dayOffset > 365)) { //说明用户输入的那个日期距离它前面的阳历1月1日已经有了超过一年的时间
            year += 1;
        ...
        for (i = 1; i < 13; i++) {
            int iPos = DAYS_BEFORE_MONTH[i];//记录公历某月前的天数

            if (year % 4 == 0 && i > 2) {//若当年位阳历闰年，且i为2月之后的月份

                iPos += 1;//因为公历闰年的2月有29天
            }
            ...
            if (iPos >= dayOffset) {
                ...
                if (year % 4 == 0 && i > 2) {//iPos重新赋值，所以对于是否是阳历闰年的判断要重新进行
                    iPos += 1;
                }
                if (dayOffset > iPos) {
    ...
    public static final int[] solarToLunar(int year, int month, int monthDay) {
        int[] lunarDate = new int[4];   //用来储存当前计算的农历年份的信息
        Date baseDate = new GregorianCalendar(1900, 0, 31).getTime();
        Date objDate = new GregorianCalendar(year, month -1, monthDay).getTime();
        int offset = (int) ((objDate.getTime() - baseDate.getTime()) / 86400000L);//86400000L是一天的毫秒数，除以它后即使两个时间之间隔了多少天
        // 用offset减去每农历年的天数计算当天是农历第几天
        // iYear最终结果是农历的年份, offset是当年的第几天
        int iYear, daysOfYear = 0;
        for (iYear = MIN_YEAR; iYear <= MAX_YEAR && offset > 0; iYear++) {
            daysOfYear = daysInLunarYear(iYear);//取到iYear那年有多少天
            offset -= daysOfYear;//遍历，每历过一年，农历第几天就减去该农历年天数
        }
        if (offset < 0) {//表示遍历时已经历到过所在那年
            offset += daysOfYear;
            iYear--;//此时得到的就是所在的农历年
        }
        // 农历年份
        lunarDate[0] = iYear;
        int leapMonth = leapMonth(iYear);//用leapMonth来记录当前闰月月份
        // 闰哪个月,1-12
        boolean isLeap = false;
        // 用当年的天数offset,逐个减去每月（农历）的天数，求出当天是本月的第几天
        int iMonth, daysOfMonth = 0;
        for (iMonth = 1; iMonth <= 13 && offset > 0; iMonth++) {
            daysOfMonth = daysInLunarMonth(iYear, iMonth);//用daysInLunarMonth来取到该月有多少天
            offset -= daysOfMonth;

        }

        if(iMonth-2==leapMonth) {//对是否是闰月的初步判断，满足此条件，表示该月为闰月或当年闰月后面的月份
            isLeap=true;
        }
        ...
        // offset小于0时，也要校正///因为遍历过头了，要往前退一个月
        if (offset < 0) {
            offset += daysOfMonth;
            --iMonth;
        ...
        lunarDate[1] = iMonth;           //对应阴历的月
        lunarDate[2] = offset + 1;       //对应阴历的日
        lunarDate[3] = isLeap ? 1 : 0;   //对应阴历是否有闰月
        return lunarDate;
    }
    ...
    private static int daysInLunarYear(int year) {//
        ...
        for (i = 0x80000; i > 0x7; i >>= 1) {//读取每一个月的大小月信息
            if ((monthInfo & i) != 0) {     //如果读到一个月是大月，该年天数就加1
                sum += 1;
            }
        }
        return sum;                      //返回这一年有多少天
    }
    ...
    private static int daysInLunarMonth(int year, int month) {
        if ((LUNAR_INFO[year - MIN_YEAR] & (0x100000 >> month)) == 0) {/////大月返回30，小月返回29
            return 29;
        } else {
            return 30;
        }
     ...
     */
    private static int leapMonth(int year) {//输入年份返回闰月月份，若当年没有闰月，则会返回0
