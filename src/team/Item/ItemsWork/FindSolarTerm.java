...
//注意：凡闰年3月1日前闰年数要减一，即：L=[(Y-1)/4],因为小寒、大寒、立春、雨水这两个节气都小于3月1日,所以 y = y-1
    ...
    public static String daySolarTerm(long time) throws ArrayException
    {
        ...
        calendar.setTimeInMillis(time);//根据给定参数，得到一个时间点
    ...
    //输出一年所有节气
    //      for(int i=0;i<24;i++) {
    //       System.out.println(arrayList.get(i).getYear()+"年"+arrayList.get(i).getMonth()+"月"+arrayList.get(i).getDay()+"日  "+arrayList.get(i).getContent());
        //   }
