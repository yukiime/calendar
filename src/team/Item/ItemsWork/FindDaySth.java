...
public class FindDaySth {
    ...
    public static ArrayList<Schedule> findScheduleArray(long dayTime) {
        ...
        // 判断是否为空
        if (end == -1 || (arrayList.get(end).getCreateTime() < dayTime)
                || (arrayList.get(0).getCreateTime() > dayTime)) {
            return scheduleList; // 不会产生空指针
        } else if (start == 0 && end == 0) {
            scheduleList.add(arrayList.get(start));
            return scheduleList;
        }
        // 在队列中二分搜索
        else {
            while (start <= end) {
                ...
                // 在队列中寻找到了与传入数据相同的元素,其下标为mid,无须记录
                    ...
                    // 先向前搜索
                    while (mid > -1 && arrayList.get(mid).getCreateTime() == dayTime) {
